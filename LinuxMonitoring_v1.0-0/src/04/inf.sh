...
MASK=${MASK#*/}
MASK=${MASK%% *}
MASK="${head}MASK${normal} = ${desc}$(cat masck.txt | grep -w $MASK | awk '{print $2}')${normal}"
echo "$MASK"
...
echo
echo "Column 1 background = ${def_param[0]} (${color_name[0]})"
echo "Column 1 font color = ${def_param[1]} (${color_name[1]})"
echo "Column 2 background = ${def_param[2]} (${color_name[2]})"
echo "Column 2 font color = ${def_param[3]} (${color_name[3]})"